struct full_dsu{
    ...
    void question(int u, int v){ ord(u, v); queries.push_back({time, time, u, v, 1}); ++time;} // consulta se os vértices estão no mesmo grupo
    void question(){ queries.push_back({time, time, 0, 0, 1}); ++time;} // consulta a quantidade de grupos distintos
