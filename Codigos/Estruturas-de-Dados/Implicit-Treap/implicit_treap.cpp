...
namespace imp_treap {
  ...
  struct node_info {
    ...
    bool rev;
    node_info() {}
    node_info(int val) : l(0), r(0), y(rng()), size(0), val(val), acc(0), add(0), rev(false) {}
  };
  ...
  inline bool rev(node t) {
    return t ? t->rev : false;
  }
  ...
  bool remove(node& t, int pos, int add = 0) {
    ...
    bool ok;
    if (pos < imp_key) {
      ok = remove(t->l, pos, add);
    } else {
      ok = remove(t->r, pos, imp_key + 1);
    }
    pull(t);
    return ok;
  }
