...
std::vector<int> smawck(F f, const std::vector<int> &rows, const std::vector<int> &cols) {
    ...
    if ((int)std::max(rows.size(), cols.size()) <= 2) {
        ...
        for (int i = 1; i < (int)rows.size(); i += 2) newRows.push_back(rows[i]);
        auto otherAns = smawck(f, newRows, cols);
        for (int i = 0; i < (int)newRows.size(); i++) ans[2 * i + 1] = otherAns[i];
...
template <class T>
std::vector<T> MaxConvolutionWithConvexShape(
    std::vector<T> anyShape, const std::vector<T> &convexShape
) {
    ...
    auto function = [&](int i, int j) { return anyShape[j] + convexShape[i - j]; };
    ...
    for (int i = 0; i < n + m - 1; i++) ans[i] = function(i, best[i]);
    return ans;
}
